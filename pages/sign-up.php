...
<?php 
  if($_SERVER["REQUEST_METHOD"] == "POST") { // Check if the method is post if yes
    // Variable for admin
    $admin = filter_input(INPUT_POST, "username", FILTER_SANITIZE_SPECIAL_CHARS);
    ...
    if(preg_match($pattern, $admin)){
      ...
      echo "Username is Invalid! Please enter only characters (no numbers). <br>";
    }

    if(empty($admin)){  // If admin is empty
